...
from utils.general import  xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy
...
def create_dataloader_modified(path, imgsz, batch_size, opt=None, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        assert img is not None, 'Image Not Found ' + path
        h0, w0 = img.shape[:2]  # orig hw
        r = np.array(self.img_size) / max(h0, w0)  # ratio
        if r != 1:  # if sizes are not equal
            img = cv2.resize(img, (int(w0 * r), int(h0 * r)),
                             interpolation=cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_LINEAR)
        return img, (h0, w0), img.shape[:2]  # img, hw_original, hw_resized
